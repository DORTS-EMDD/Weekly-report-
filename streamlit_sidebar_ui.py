    ...
    with st.sidebar:
        ...
        with st.form("report_settings_form", clear_on_submit=False):
            ...
            if "selected_types_state" not in st.session_state:
                st.session_state["selected_types_state"] = context.default_selected_types.copy()
            if (
                not st.session_state.get("_default_types_without_standards_applied")
                and st.session_state.get("selected_types_state") == context.advanced_types
            ):
                st.session_state["selected_types_state"] = context.default_selected_types.copy()
                st.session_state["type_規範更新"] = False
            ...
            if "selected_regions_state" not in st.session_state:
                st.session_state["selected_regions_state"] = context.default_regions.copy()
            ...
            if not global_scope_selected:
                st.caption(
                    f"已選 {len(stored_selected_regions)} / {len(context.advanced_regions)} 個國家"
                )

            def select_all_regions() -> None:
                st.session_state["selected_regions_state"] = context.advanced_regions.copy()
            ...
            if not global_scope_selected and not selected_regions:
            ...
            with st.expander("⚙️ 進階設定", expanded=False):
                include_research_supplement = st.checkbox(
                    "國際捷運技術期刊",
                    value=bool(st.session_state.get("include_research_supplement", False)),
