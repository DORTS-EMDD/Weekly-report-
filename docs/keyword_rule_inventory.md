...
**A8 coverage amendment:** `FREEZE_COVERAGE_UPDATE_REQUIRED` was raised for the
Jakarta geography case. The executable geography owner now includes general
Jakarta/Indonesia metro evidence; this section records that active coverage.
...
| Rule ID | Classification | Owner file / function / constant | Active | Positive / negative terms | Regex / pattern | Language / query / domain | Precedence | Output field | Consumers | Fallback | Duplicate owner | Collision / coverage gap | Notes |
|---|---|---|---|---|---|---|---:|---|---|---|---|---|---|
| GEO-001 | GEOGRAPHY_EVIDENCE | `config.py`: `EVENT_REGION_PRIORITY_HINTS` | YES | Taipei/New Taipei/Taoyuan/Taichung/Kaohsiung; Jakarta/Indonesia; Sydney, Basel, Zürich, Lausanne, Austin, Houston, Vancouver, Toronto, Berlin, Leipzig, Munich | normalized term spans | multilingual article text | 10 | event-location evidence | `article_processor._region_resolution_details` | NO | NO | City/system collision guarded by owner | Jakarta/Indonesia aliases are general evidence, not a selector case. |
| GEO-002 | GEOGRAPHY_EVIDENCE | `config.py`: `METRO_SYSTEM_OWNERSHIP_RULES` | YES | Jakarta MRT/PT MRT Jakarta plus existing location/operator/system aliases | rule tuple matching | Jakarta, Sydney, München, Vienna, Manchester, Chennai | 20 | metro-system evidence | article processor | NO | NO | Manufacturer/test location cannot override event/system | Jakarta high-precedence evidence requires an MRT/system anchor. |
...
| GEO-005 | NORMALIZATION | `article_processor.py`: `normalize_country`, source/domain normalizers | YES | Taiwan/臺灣, Indonesia/Indonesian/印尼/印度尼西亞 and country spelling variants; source label noise | casefold/alias maps | all providers | before evidence resolution | normalized country/domain/source | every candidate consumer | NO | NO | Generic source labels remain display-only | Jakarta coverage amendment is owned by executable geography data. |
