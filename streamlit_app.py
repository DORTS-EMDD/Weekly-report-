...
8. **數量原則（目標固定 10 則）**：請以「輸出 10 則」為目標。若第一輪嚴格篩選後不足 10 則，可在不違反上述查核原則（地區排除、時間範圍、不納入高鐵/貨運鐵路）的前提下，放寬「次要相關性」標準（例如同類別中關聯度較低、但仍屬都市軌道交通範疇的事件）來補足；**但絕對不可捏造、虛構或挪用非本週時間範圍的新聞來湊數**。若放寬後仍不足 10 則，請如實輸出實際則數，並在結尾統計處註明「本週實際符合條件共 N 則，未達 10 則目標之原因：（資料不足／來源無更新等）」。
...
🔍 **執行搜尋次數**：Google News 代理 {len(GNEWS_SOURCES)} 源 + ddgs {search_count} 次關鍵字搜尋
