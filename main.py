"""
國際捷運技術週報自動產生器
- 使用 Gemini API (含 Google Search grounding)
- 每週一自動執行，寄送報告至指定信箱
"""
...
GMAIL_USER       = os.environ["GMAIL_USER"]       # 你的 Gmail 帳號
GMAIL_APP_PASS   = os.environ["GMAIL_APP_PASS"]   # Gmail 應用程式密碼
RECIPIENTS       = os.environ["RECIPIENTS"]        # 逗號分隔，多個收件人

MODEL_NORMAL     = "gemini-3.1-flash-lite"         # 平日省配額
MODEL_POWERFUL   = "gemini-3.5-flash"              # 複雜任務備用
...
def build_prompt() -> str:
    return f"""
# 任務
你是專業的捷運機電技術分析師。請使用 search 工具，在今日（{today.strftime('%Y年%m月%d日')}）執行至少 12 次獨立搜尋，產出過去 7 天（{date_range}）的國際軌道交通重大資訊週報。
...
3. **免費公開原則**：禁止引用付費牆網站（Railway Gazette 鎖碼文章等），優先使用官方新聞稿
4. **寧缺勿濫**：若窮盡搜尋後無符合條件新聞，直接回報「系統監測正常，本週無符合條件之重大異動」

## 核心內容（三大領域）
1. **技術新知與政策**（次世代與首創）：GoA4、CBTC、FRMCS/5G/6G鐵道通訊、列車虛擬聯結、AI預防性維修、數位雙生、邊緣運算、SiC半導體牽引、超級電容、氫能、EULYNX標準化
...
* **【臺北捷運啟示】**：（對北捷系統防護或營運管理的具體參考價值；若無關聯請寫「暫無直接關聯」）

---
（重複以上區塊，無數量上限，但每則都必須符合 7 天內條件）

## 結尾固定格式
---
📊 **本週統計**：共 N 則符合條件之國際資訊
🔍 **執行搜尋次數**：本次共執行 N 次搜尋
⏰ **報告產出時間**：{today.strftime('%Y年%m月%d日')} 週一
"""
...
def generate_report(use_powerful_model: bool = False) -> str:
    ...
    model = genai.GenerativeModel(
        ...
        tools="google_search_retrieval",   # 啟用 Google Search grounding
    )
...
def send_email(report_text: str, recipients: list[str]) -> bool:
    """寄送 HTML 格式報告"""
...
if __name__ == "__main__":
    import sys
    powerful = "--powerful" in sys.argv
    main(use_powerful=powerful)
